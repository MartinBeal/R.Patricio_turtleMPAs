...
for(y in seq_along(datatypes)){
  ...
  if(datatype == "interpolated"){
    ...
    # count based on % probability from dBBMMs for each ID
    migrid <- readRDS("data/analysis/mig_grid/raw_filtered_dbbmm_mig_grid_10x10_21.rds")
    # count based just on raw, filtered points
  ...
  # ggsave(paste0("figures/migrid_10x10_n22dbbmm_", datatype, ".png"), 
  #        plot=map, width = 5, height=10 )
  }
